...
public abstract class JPaymentSelect extends javax.swing.JDialog
    ...
    public boolean showDialog(double total, CustomerInfoExt customerext,double deposit) {
        ...
        m_jTotalEuros.setText(Formats.CURRENCY.formatValue(Math.ceil(m_dTotal * 20) / 20));
...
// Get the size of the screen, and center the dialog in the window
    ...
    protected void addTabPayment(JPaymentCreator jpay) {
        if (app.getAppUserView().getUser().hasPermission(jpay.getKey())) {
            ...
            m_jTabPayment.addTab(null, jpayinterface.getComponent());
            m_jTabPayment.setTabComponentAt(m_jTabPayment.getTabCount() - 1, label);
        }
    }
    ...
    private void printState() {
        ...
        double dRemaining = Math.ceil((m_dTotal - m_aPaymentInfo.getTotal()) * 20) / 20;
        m_jRemaininglEuros.setText(Formats.CURRENCY.formatValue(dRemaining));
