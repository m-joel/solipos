...
public class AppConfig implements AppProperties {
    ...
    public void setLicenseKey(String licenseKey) throws IOException {
        pullLatestProperties();
        properties.setProperty("machine.licenseKey", licenseKey);
    ...
    private void setDeviceProperties(Properties properties) {
        ...
        properties.setProperty("swing.defaultlaf",
    ...
    // Generate a license key from the serial number using SHA-256
    public String generateKey(String serialNumber) {
        try {
            MessageDigest sha = MessageDigest.getInstance("SHA-256");
            byte[] keyBytes = sha.digest(serialNumber.getBytes());
    ...
    public boolean keyCheck(String checkKey) {
        String deviceSerial = getDeviceSerialNumber();
        if (deviceSerial == null || deviceSerial.isEmpty()) {
            return false;
        }

        String correctKey = generateKey(deviceSerial);

        return checkKey != null && checkKey.equals(correctKey);
    }
}
